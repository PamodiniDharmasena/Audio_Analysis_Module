...
# Define categories and their expected object interactions
CATEGORY_DEFINITIONS = {
    '1_baseball_bat_attack': {
        'required_objects': ['person', 'baseball bat'],
        'interaction_threshold': 50,
        'min_objects': 2
    },
    '2_bomb_explosion_in_public': {
    ...
    '3_hit_and_run': {
        'required_objects': ['person', 'car', 'truck', 'bus'],
        'speed_threshold': 30,
        'distance_threshold': 100,
        'min_objects': 2
    },
    '4_kill_cow_with_knife': {
        'required_objects': ['person', 'cow', 'pig', 'dog', 'knife'],
        'distance_threshold': 30,
        'min_objects': 3
    },
    '5_lip_kissing': {
        'required_objects': ['person', 'person'],
...
def get_object_group(obj_type):
    """Categorize object into broader groups"""
    for group, types in OBJECT_GROUPS.items():
        if obj_type in types:
            return group
    return obj_type

def calculate_movement_features(position_history, frame_rate):
    """Calculate velocity, acceleration, and movement patterns"""
    ...
    movement_variance = np.var(directions) if directions else 0
    
    return {
        'avg_velocity': float(np.mean(velocities)) if velocities else 0,
        'max_velocity': float(np.max(velocities)) if velocities else 0,
        'avg_acceleration': float(np.mean(accelerations)) if len(accelerations) > 0 else 0,
        'movement_variance': float(movement_variance)
    }

def analyze_interactions(tracked_objects, frame_rate):
    """Analyze interactions between objects"""
    interactions = []
    obj_ids = list(tracked_objects.keys())
    
    # Pairwise interactions
    for obj1_id, obj2_id in combinations(obj_ids, 2):
        obj1_data = tracked_objects[obj1_id]
        obj2_data = tracked_objects[obj2_id]
        
        common_frames = min(len(obj1_data['positions']), len(obj2_data['positions']))
        if common_frames == 0:
            continue
        
        distances = []
        for k in range(common_frames):
            x1, y1 = obj1_data['positions'][k]
            x2, y2 = obj2_data['positions'][k]
            distance = np.sqrt((x2-x1)**2 + (y2-y1)**2)
            distances.append(distance)
        
        interactions.append({
            'object1': obj1_data['type'],
            'object2': obj2_data['type'],
            'object1_id': obj1_id,
            'object2_id': obj2_id,
            'avg_distance': float(np.mean(distances)) if distances else 0,
            'min_distance': float(np.min(distances)) if distances else 0,
            'frames_together': common_frames
        })
    
    return interactions

def detect_category_features(category, tracked_objects, interactions, video_data):
    """Detect features specific to each category"""
    features = {}
    cat_def = CATEGORY_DEFINITIONS.get(category, {})
    
    # Fire analysis for bomb explosions
    if category == '2_bomb_explosion_in_public':
        fire_frames = [frame for frame in video_data if frame.get('fire_detected', False)]
        fire_volumes = [frame.get('fire_volume', 0) for frame in video_data]
        
        features.update({
            'fire_detected': len(fire_frames) > 0,
            'fire_frames': len(fire_frames),
            'max_fire_volume': max(fire_volumes) if fire_volumes else 0,
            'avg_fire_volume': np.mean(fire_volumes) if fire_volumes else 0,
            'fire_threshold_exceeded': max(fire_volumes) > cat_def.get('fire_threshold', 0) if fire_volumes else False
        })
    
    # People count for all categories
    people_count = sum(1 for obj in tracked_objects.values() if obj['type'] == 'person')
    features['people_count'] = people_count
    
    # Category-specific interaction analysis
    if category == '1_baseball_bat_attack':
        bat_interactions = [i for i in interactions 
                          if 'baseball bat' in [i['object1'], i['object2']]]
        features['bat_interactions'] = bat_interactions
    
    elif category == '3_hit_and_run':
        vehicle_interactions = [i for i in interactions 
                              if any(v in get_object_group(i['object1']) or 
                                 v in get_object_group(i['object2']) 
                              for v in ['vehicle'])]
        features['vehicle_interactions'] = vehicle_interactions
    
    elif category == '4_kill_cow_with_knife':
        knife_interactions = [i for i in interactions 
                            if 'knife' in [i['object1'], i['object2']]]
        features['knife_interactions'] = knife_interactions
    
    elif category == '5_lip_kissing':
        person_person_interactions = [i for i in interactions 
                                    if i['object1'] == 'person' and i['object2'] == 'person']
        features['person_interactions'] = person_person_interactions
    
    return features

def extract_features_from_video(video_data, category):
    """Extract meaningful features from video data"""
    frame_rate = 30
    if len(video_data) > 1:
        frame_rate = 1 / ((video_data[1]['timestamp'] - video_data[0]['timestamp']) 
                         if 'timestamp' in video_data[0] else 1/30)
    ...
    for frame in video_data:
        for obj in frame.get('objects', []):
            obj_id = obj['id']
            tracked_objects[obj_id]['type'] = obj['type']
            tracked_objects[obj_id]['positions'].append((obj['x'], obj['y']))
            if tracked_objects[obj_id]['first_frame'] is None:
                tracked_objects[obj_id]['first_frame'] = frame['frame']
            tracked_objects[obj_id]['last_frame'] = frame['frame']
    
    # Calculate movement features
    for obj_id in tracked_objects:
        movement_features = calculate_movement_features(
            tracked_objects[obj_id]['positions'], 
            frame_rate
        )
        tracked_objects[obj_id]['movement'] = movement_features
    
    # Analyze interactions
    interactions = analyze_interactions(tracked_objects, frame_rate)
    ...
    # Count objects and their presence
    for obj_id, obj_data in tracked_objects.items():
        obj_type = obj_data['type']
        features['object_counts'][obj_type] += 1
        features['object_presence'][obj_type] = obj_data['last_frame'] - obj_data['first_frame']
        
        # Store tracked object details
        features['tracked_objects'][obj_id] = {
            'type': obj_type,
    ...
    features.update(detect_category_features(
        category, 
        tracked_objects, 
        interactions, 
        video_data
    ...
    required_objects = CATEGORY_DEFINITIONS.get(category, {}).get('required_objects', [])
    present_objects = set(obj['type'] for obj in tracked_objects.values())
    
    features['requirements_met'] = all(
        obj in present_objects 
        for obj in required_objects
    )
    
    return features

def process_features(input_path, output_path, category):
    """Process a single video's features and save to output"""
    try:
        with open(input_path, 'r') as f:
            video_data = json.load(f)
        
        features = extract_features_from_video(video_data, category)
        
        with open(output_path, 'w') as f:
            json.dump(features, f, indent=4)
        
        print(f"Processed features: {input_path} -> {output_path}")
        return True
    except Exception as e:
        print(f"Error processing features {input_path}: {str(e)}")
        return False
...
class VideoAnalyzerApp:
    def __init__(self, root):
        ...
        # Open YouTube in browser when app starts
        self.open_youtube()
    ...
    def create_left_column(self):
        ...
        # Paste button
        self.paste_button = ttk.Button(self.url_frame, text="Paste", command=self.paste_from_clipboard)
        ...
        self.features_frame = self.make_progress_section("3. Extracting features...", "Waiting")
        self.audio_frame = self.make_progress_section("4. Analyzing audio content...", "Waiting")
        self.comment_frame = self.make_progress_section("5. Analyzing comment content...", "Waiting")
        ...
        # Features Results Tab
        self.features_tab = ttk.Frame(self.notebook)
        self.features_text = tk.Text(self.features_tab, wrap=tk.WORD, state='disabled', padx=5, pady=5)
        self.features_scroll = ttk.Scrollbar(self.features_tab, command=self.features_text.yview)
        self.features_text.config(yscrollcommand=self.features_scroll.set)
    ...
    def create_right_column(self):
        ...
        self.features_button = ttk.Button(self.button_frame, text="View Features", 
                                       command=self.show_features, state=tk.DISABLED, width=15)
        self.features_button.pack(side=tk.TOP, fill=tk.X, pady=2)
    ...
    def setup_layout(self):
        ...
        self.notebook.add(self.features_tab, text="Extracted Features")
        ...
        self.features_scroll.pack(side=tk.RIGHT, fill=tk.Y)
    ...
    def open_youtube(self):
        ...
        messagebox.showinfo("Instructions", 
                          "Please browse YouTube and click on a video to copy its URL.\n"
                          "The application will automatically detect YouTube URLs in your clipboard.\n"
                          "You can also paste the URL manually and click 'Start Analysis'.")
    ...
    def monitor_clipboard(self):
        ...
        try:
            ...
            # Only proceed if clipboard content has changed and is a YouTube URL
            if (current_clipboard != self.last_clipboard_content and 
                self.is_youtube_url(current_clipboard)):
                
                self.last_clipboard_content = current_clipboard
                self.url_entry.delete(0, tk.END)
                self.url_entry.insert(0, current_clipboard)
                
                # Optional: Uncomment the next line to automatically start analysis
                # self.start_analysis()
        ...
        if self.clipboard_monitor_active:
            self.root.after(1000, self.monitor_clipboard)  # Check every second
    ...
    def paste_from_clipboard(self):
        ...
        try:
            ...
            if self.is_youtube_url(clipboard_content):
                self.url_entry.delete(0, tk.END)
    ...
    def start_analysis(self):
        ...
        self.features_button.config(state=tk.DISABLED)
        ...
        try:
            ...
            self.update_step(self.video_frame, "Extracting video content...", 0)
    ...
    def run_video_processing(self, video_path, video_id):
        try:
            ...
            json_output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'json_data')
            os.makedirs(json_output_dir, exist_ok=True)
            
            # Process the video (this will take some time)
            video_data = process_video(video_path, 'none', 'none')  # Using 'none' category for downloaded videos
            
            # Save the extracted features to JSON
            json_output_path = os.path.join(json_output_dir, f"{video_id}.json")
            save_to_json(video_data, json_output_path)
            ...
            # Process features in a separate step
            self.root.after(0, lambda: self.update_step(
                self.features_frame,
            ...
            # Process features
            features_output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'json_data_extracted')
            os.makedirs(features_output_dir, exist_ok=True)
            
            features_output_path = os.path.join(features_output_dir, f"{video_id}.json")
            success = process_features(json_output_path, features_output_path, 'none')
            ...
            if success:
                ...
                self.root.after(0, self.continue_analysis, video_path, video_id, features_output_path)
            else:
                self.root.after(0, lambda: self.update_step(
                    self.features_frame,
    ...
    def continue_analysis(self, video_path, video_id, features_path):
        try:
            # Video Analysis
            ...
            self.current_results = {
                ...
                'video_features_path': features_path  # Add path to the extracted features
            }
            ...
            if any([video_pred, audio_pred, comment_pred]):
                ...
                self.features_button.config(state=tk.NORMAL)
    ...
    def show_features(self):
        if not hasattr(self, 'current_results') or not self.current_results.get('video_features_path'):
            messagebox.showerror("Error", "No features available to display")
            return
            
        try:
            with open(self.current_results['video_features_path'], 'r') as f:
            ...
            formatted_features = json.dumps(features, indent=4)
            self.features_text.insert(tk.END, formatted_features)
    ...
    def reset_analysis(self):
        for frame in [self.download_frame, self.video_frame, self.features_frame, 
                     self.audio_frame, self.comment_frame]:
        ...
        self.features_text.config(state='normal')
        self.features_text.delete(1.0, tk.END)
        self.features_text.insert(tk.END, "Run analysis first, then click 'View Features'")
        self.features_text.config(state='disabled')
